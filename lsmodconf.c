...
int main(int argc, char **argv)
{
    ...
    while (fgets(mod_line, LINE_MAX, stdin) != NULL) {
        ...
        if (strcmp(mod_name, "Module") == 0) continue;

        config_name(mod_name, conf_name);

        // Handle unknown module names
        if (conf_name[0] == '\0') {
            printf(ANSI_COLOR_YELLOW "# Module %s is not known to lsmodconf" ANSI_COLOR_RESET "\n", mod_name);
            continue;
        }
        ...
        printf(ANSI_COLOR_RED "# Config string %s for module %s could not be found. Perhaps there's a version mismatch?" ANSI_COLOR_RESET "\n",
                        conf_name, mod_name);
    }
}
